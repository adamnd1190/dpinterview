...
def fetch_interviews(
    ...
    for interview_type in interview_types:
        interview_type_path = (
            study_path / subject_id / interview_type.value / "raw" 
        )
...
{interview_type_path} does not exist."
        ...
        for interview_dir in interview_dirs:
            ...
            try:
                # Handle your date format: 27AUG2025
                date_dt = datetime.strptime(parts[1], "%d%b%Y").date()
            except ValueError:
                try:
                    # Fallback formats
                    ...
                    try:
                        ...
                        logger.error(f"Could not parse date for {base_name}. Expected format like '27AUG2025'")
                        continue

            # Handle missing time components
            if len(parts) >= 5:
                try:
            ...
            interview_name = dpdash.get_dpdash_name(
                study=study_id,
                subject=subject_id,
                data_type="onsiteInterview" if interview_type == InterviewType.ONSITE else "offsiteInterview",
...
def generate_queries(
    ...
    for interview_part in interview_parts:
        ...
        # Extract the interview type from the data_type
